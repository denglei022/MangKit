...
extension UIWindow {
    override open func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        if MK.sharedInstance().getSelectedGesture() == .shake {
            if (event!.type == .motion && event!.subtype == .motionShake) {
                MK.sharedInstance().motionDetected()
            }
        } else {
...
extension DataCleaner where Self: UIViewController {
    ...
    func clearData(sourceView: UIView, originingIn sourceRect: CGRect?, then: @escaping () -> ()) {
        let actionSheetController: UIAlertController = UIAlertController(title: "确定清除缓存?", message: "", preferredStyle: .actionSheet)
        ...
        let yesAction = UIAlertAction(title: "确定", style: .default) { _ in
            MK.sharedInstance().clearOldData()
            then()
        }
        actionSheetController.addAction(yesAction)

        let noAction = UIAlertAction(title: "取消", style: .default) { _ in }
        actionSheetController.addAction(noAction)

        present(actionSheetController, animated: true, completion: nil)
    }
    
    func clearLog(sourceView: UIView, originingIn sourceRect: CGRect?, then: @escaping () -> ()) {
        let actionSheetController: UIAlertController = UIAlertController(title: "确定清除缓存?", message: "", preferredStyle: .actionSheet)
        actionSheetController.popoverPresentationController?.sourceView = sourceView
        if let sourceRect = sourceRect {
            actionSheetController.popoverPresentationController?.sourceRect = sourceRect
        ...
        let yesAction = UIAlertAction(title: "确定", style: .default) { _ in
            MK.sharedInstance().clearLogData()
            then()
        }
        actionSheetController.addAction(yesAction)
