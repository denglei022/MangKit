...
// 声明协议
@objc protocol FloatDelegate :NSObjectProtocol{
    
    @objc optional func singleClick()
    
    @objc optional func repeatClick()

}

class MKFloatingBtn: UIButton {
    ...
    @objc func singleClickMethod() {
        if self.delegate != nil && self.delegate!.responds(to: #selector(FloatDelegate.singleClick)){
            self.delegate?.singleClick!()
        }
    }
    
    @objc func repeatClickMethod() {
        if self.delegate != nil && self.delegate!.responds(to: #selector(FloatDelegate.repeatClick)){
