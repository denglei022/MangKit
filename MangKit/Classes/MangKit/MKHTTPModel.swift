...
@objc public class MKHTTPModel: NSObject {
    ...
    func saveRequest(_ request: URLRequest) {
        ...
        requestURL = request.getMKURL()
        requestURLComponents = request.getMKURLComponents()
        requestURLQueryItems = request.getMKURLComponents()?.queryItems
        requestMethod = request.getMKMethod()
        requestCachePolicy = request.getMKCachePolicy()
        requestTimeout = request.getMKTimeout()
        requestHeaders = request.getMKHeaders()
        requestType = requestHeaders?["Content-Type"] as! String?
        requestCurl = request.getCurl()
    }
    
    func saveRequestBody(_ request: URLRequest) {
        saveRequestBodyData(request.getMKBody())
    }
    
    func logRequest(_ request: URLRequest) {
        formattedRequestLogEntry().appendToFileURL(MKPath.sessionLogURL)
    }
    ...
    func saveResponse(_ response: URLResponse, data: Data) {
        ...
        responseStatus = response.getMKStatus()
        responseHeaders = response.getMKHeaders()
        
        let headers = response.getMKHeaders()
        ...
        formattedResponseLogEntry().appendToFileURL(MKPath.sessionLogURL)
    }
    ...
    @objc public func getRequestBodyFileURL() -> URL {
        return MKPath.pathURLToFile(getRequestBodyFilename())
    }
    ...
    @objc public func getResponseBodyFileURL() -> URL {
        return MKPath.pathURLToFile(getResponseBodyFilename())
    }
    ...
    @objc public func saveData(_ dataString: String, to fileURL: URL) {
        do {
            ...
            print("[MK]: Failed to save data to [\(fileURL)] - \(error.localizedDescription)")
        }
    }
    ...
    @objc public func readRawData(from fileURL: URL) -> Data? {
        do {
            ...
            print("[MK]: Failed to load data from [\(fileURL)] - \(error.localizedDescription)")
            return nil
        }
    }
